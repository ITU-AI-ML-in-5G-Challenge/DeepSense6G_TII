...
#                 print('before full lidar',full_lidar[i][:,0].max(),full_lidar[i][:,0].min())
                full_lidar[i] = transform_2d_points(full_lidar[i], 
                    np.pi/2-seq_theta[i], -seq_x[i], -seq_y[i], np.pi/2-ego_theta, -ego_x, -ego_y)
#                 print('afetr full lidar',full_lidar[i][:,0].max(),full_lidar[i][:,0].min())
                
                lidar_processed = lidar_to_histogram_features(full_lidar[i], crop=self.input_resolution)
#                 print('processed',lidar_processed[0,:].max(),lidar_processed[0,:].min())
                
                data['lidars'].append(lidar_processed)
#                 print('processed',lidar_processed[1,:,:].max(),lidar_processed[1,:,:].min())
