...
class TransformerDecorator(nn.Module):
    def __init__(self, add_bf=0, dim=128):
        super(TransformerDecorator, self).__init__()
        self.encoder_layers = nn.TransformerEncoderLayer(dim, 4, dim, 0.5)
        self.add_bf = add_bf
...
#         print('transfo',self.config.n_views , self.seq_len)
        ...
        lidar_tensor_out = x[:, self.config.n_views*self.seq_len:self.config.n_views*self.seq_len+1, :, :, :].contiguous().view(bz * self.seq_len, -1, h, w)
        radar_tensor_out = x[:, self.config.n_views*self.seq_len+1:, :, :, :].contiguous().view(bz * self.seq_len, -1, h, w)
        return image_tensor_out, lidar_tensor_out, radar_tensor_out
...
class Encoder(nn.Module):
    ...
    def forward(self, image_list, lidar_list, radar_list, velocity):
        ...
        image_features_layer1, lidar_features_layer1, radar_features_layer1 = self.transformer1(image_embd_layer1, lidar_embd_layer1, radar_embd_layer1, velocity)
        ...
        image_features_layer2, lidar_features_layer2, radar_features_layer2 = self.transformer2(image_embd_layer2, lidar_embd_layer2, radar_embd_layer2, velocity)
        ...
        image_features_layer3, lidar_features_layer3, radar_features_layer3 = self.transformer3(image_embd_layer3, lidar_embd_layer3, radar_embd_layer3, velocity)
        ...
        image_features_layer4, lidar_features_layer4, radar_features_layer4 = self.transformer4(image_embd_layer4, lidar_embd_layer4, radar_embd_layer4, velocity)
...
class TransFuser(nn.Module):
    ...
    def __init__(self, config, device, add_bf=0):
        super().__init__()
        self.device = device
        self.config = config
        ...
        self.encoder_global = TransformerDecorator(self.add_bf, 128)
        if self.add_bf:
